...
#import <Keys/SkywayIosSdkTutorialKeys.h>
...
- (void)viewDidLoad {
    ...
    // Initialize Peer
    //
    SkywayIosSdkTutorialKeys* keys = [[SkywayIosSdkTutorialKeys alloc] init];
    SKWPeerOption* option = [[SKWPeerOption alloc] init];
    option.key = keys.skyWayApiKey;
    option.domain = keys.skyWayDomain;
    option.debug = 3;
...
//
- (IBAction)onSwitchCameraButtonClicked:(id)sendor {
    if (nil == _localStream) {
        return;
    }
